...
PKG_BUILDER=$1 su -c "alps install-no-prompt sudo"
if ! grep "sudo=" /etc/alps/installed-list &> /dev/null
then
        echo "Essentials incomplete (sudo). Aborting..."
        exit 1
fi
su - $USERNAME -c "PKG_BUILDER=$1 alps install-no-prompt usbutils pciutils openssh gobject-introspection libxml2 desktop-file-utils shared-mime-info ccache"
